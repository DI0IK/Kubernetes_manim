...
class GitopsCLI(Slide):
    def construct(self):
        ...
        kubectl_sub = Text("Imperativ / CLI", font_size=14, color=TEXT_MUTED).next_to(kubectl_title, DOWN, buff=0.1).align_to(kubectl_title, LEFT)
        
        cmds = VGroup(
            Text("$ kubectl apply -f deploy.yaml", font_size=14, color=STORE_YELLOW, font="Monospace"),
            Text("$ kubectl get pods -w", font_size=14, color=STORE_YELLOW, font="Monospace"),
        ...
        k9s_sub = Text("Terminal-UI (TUI)", font_size=14, color=TEXT_MUTED).next_to(k9s_title, DOWN, buff=0.1).align_to(k9s_title, LEFT)
        
        k9s_desc = Text("Schnelle Cluster-Navigation\n& Live-Metriken", font_size=16, color=TEXT_LIGHT, line_spacing=1.5).next_to(k9s_sub, DOWN, buff=0.6).align_to(k9s_title, LEFT)
        ...
        # Simuliertes Web-UI Element
        argo_ui_box = RoundedRectangle(width=3.7, height=1.6, corner_radius=0.1, color=STORE_YELLOW, stroke_width=1, fill_color=STORE_YELLOW, fill_opacity=0.15).next_to(argo_title, DOWN, buff=0.3).align_to(argo_title, LEFT)
        argo_desc = Text("Fokus auf Web-UI &\nVisuelles Dashboard", font_size=16, color=TEXT_LIGHT, line_spacing=1.2).move_to(argo_ui_box)
        
        argocd_g = VGroup(argo_base, argo_title, argo_ui_box, argo_desc).shift(LEFT*3.5 + DOWN*0.2)
        ...
        # Simuliertes Code/CRD Element
        flux_crd_box = RoundedRectangle(width=3.7, height=1.6, corner_radius=0.1, color=K8S_BLUE, stroke_width=1, fill_color=K8S_BLUE, fill_opacity=0.15).next_to(flux_title, DOWN, buff=0.3).align_to(flux_title, LEFT)
        flux_desc = Text("K8s-nativ via CRDs\n(Kustomize/Helm)", font_size=16, color=TEXT_LIGHT, line_spacing=1.2).move_to(flux_crd_box)
        
        flux_g = VGroup(flux_base, flux_title, flux_crd_box, flux_desc).shift(RIGHT*3.5 + DOWN*0.2)

        self.play(
            LaggedStart(
                AnimationGroup(DrawBorderThenFill(argo_base), Write(argo_title), DrawBorderThenFill(argo_ui_box), FadeIn(argo_desc)),
                AnimationGroup(DrawBorderThenFill(flux_base), Write(flux_title), DrawBorderThenFill(flux_crd_box), FadeIn(flux_desc)),
                lag_ratio=0.3
            )
        )
        self.next_slide()
        ...
        self.play(FadeOut(argocd_g), FadeOut(flux_g), Transform(title, outro))
